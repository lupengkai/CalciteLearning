 ...
 */
package com.tage.calcite.adapter.druid;

import com.google.common.base.Predicate;
import com.google.common.collect.ImmutableList;
import com.google.common.collect.Iterables;
import com.tage.calcite.adapter.druid.DruidQuery;
...
/**
 * Rules and relational operators for {@link com.tage.calcite.adapter.druid.DruidQuery}.
 */
public class DruidRules {
  ...
  /**
   * Rule to push an {@link Aggregate}
   * into a {@link com.tage.calcite.adapter.druid.DruidQuery}.
   */
  private static class DruidAggregateRule extends RelOptRule {
    private DruidAggregateRule() {
      super(
          operand(Aggregate.class,
              operand(com.tage.calcite.adapter.druid.DruidQuery.class, none())));
    }

    public void onMatch(RelOptRuleCall call) {
      final Aggregate aggregate = call.rel(0);
      final com.tage.calcite.adapter.druid.DruidQuery query = call.rel(1);
      if (aggregate.indicator
          || aggregate.getGroupSets().size() != 1
          || Iterables.any(aggregate.getAggCallList(), BAD_AGG)) {
        return;
      }
      if (!com.tage.calcite.adapter.druid.DruidQuery.isValidSignature(query.signature() + 'a')) {
        return;
      }
  ...
  private static class DruidProjectRule extends RelOptRule {
    ...
    public void onMatch(RelOptRuleCall call) {
      ...
      final com.tage.calcite.adapter.druid.DruidQuery query = call.rel(1);
      if (!com.tage.calcite.adapter.druid.DruidQuery.isValidSignature(query.signature() + 'p')) {
        return;
      }
      if (com.tage.calcite.adapter.druid.DruidQuery.canProjectAll(project.getProjects())) {
      ...
      final Pair<List<RexNode>, List<RexNode>> pair =
              com.tage.calcite.adapter.druid.DruidQuery.splitProjects(project.getCluster().getRexBuilder(), query,
              project.getProjects());
      if (pair == null) {
      ...
      final com.tage.calcite.adapter.druid.DruidQuery newQuery = extendQuery(query, newProject);
  ...
  private static class DruidFilterRule extends RelOptRule {
    ...
    public void onMatch(RelOptRuleCall call) {
      ...
      final com.tage.calcite.adapter.druid.DruidQuery query = call.rel(1);
      if (!com.tage.calcite.adapter.druid.DruidQuery.isValidSignature(query.signature() + 'f')
  ...
  public static com.tage.calcite.adapter.druid.DruidQuery extendQuery(com.tage.calcite.adapter.druid.DruidQuery query, RelNode r) {
